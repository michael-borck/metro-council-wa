...
"""Build the Metro Council WA bespoke site.
...
Usage:
    ...
    ./site/build.py serve        # build then serve on :8005 for local preview
...
def build():
    print(f"Building Metro Council WA → {DIST_DIR}")
    ...
    if not cname_file.exists():
        cname_file.write_text("metrocouncilwa.eduserver.au\n", encoding="utf-8")
        print("  ✓ CNAME")
...
def serve():
    """Build then serve the dist directory on :8005 for local preview."""
    import http.server
    import socketserver
    ...
    with socketserver.TCPServer(("", PORT), http.server.SimpleHTTPRequestHandler) as httpd:
        print(f"\nServing Metro Council at http://localhost:{PORT}")
